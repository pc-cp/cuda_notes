...
// 48 KB of SMEM per SM
// 1536 threads per SM
// Thus for all 1536 thread slots to be used, a thread block should not be use more than an average of (48 KB)/(1536 threads) = 32 B/thread. In the tiled matrix mul, each thread use 8 B/thread. Therefore the tiled matrix mul kernel's occupancy is not limited by the SMEM.

template <int kTile>
__global__ void matrix_mul_tile_v1(float* P, float *M, float *N, int j, int k, int l) {
	...
	for (int phase = 0; phase < phases; ++phase) {
		...
		Ns[threadIdx.y][threadIdx.x] = N[(phase * kTile + threadIdx.y) * l + Col];
		__syncthreads(); // RAW
		for (int t = 0; t < kTile; ++t) {
			Pvalue += Ms[threadIdx.y][t] * Ns[t][threadIdx.x];
		}
		__syncthreads(); // WAR
	}
...
template <int kTile>
__global__ void matrix_mul_tile_v2(float* P, float *M, float *N, int j, int k, int l) {
	__shared__ float Ms[kTile][kTile];
	__shared__ float Ns[kTile][kTile];
...
TEST(matrix_mul_tile, demo0) {
	...
	matrix_mul_tile_v1<kTile><<<grid, block>>>(P_d, M_d, N_d, j, k, l);
	CUDA_CHECK_KERNEL();
	...
	// why kTile == 16 faster kTile == 32 if kTile == 32 use less sync and high compute-intensity, kTile == 32 lead less blocks in SM, so reduce tolerate long latency?
	
	// kTile = 16
	printf("matrix_mul_tile_v1: %.3f ms/iters\n", ms/iters);
	// matrix_mul_tile_v1: 0.804 ms/iters
	
	// kTile = 32
	// matrix_mul_tile_v1: 0.827 ms/iters
	
	// kTile = 8;
	// matrix_mul_tile_v1: 1.057 ms/iters
